...
# CORDIC variation of the angle
sigma = [0.7853982, 0.4636476, 0.2449787, 0.1243550, 0.0624188, 0.0312398, 0.0156237]


# 8x8 matrix constant operation: values are all 32-bit integer
def op(matrix_in:list, option:str, integer:int):
    index = 0
    matrix = [0] * 64
    if option == '-':
        while index != 64:
            matrix[index] = matrix_in[index] - integer
            index += 1
    elif option == '*':
        while index != 64:
            matrix[index] = matrix_in[index] * integer
        ...
        while index != 64:
            matrix[index] = int(matrix_in[index] / integer)
            index += 1
    return matrix
...
def dct(s:list):
    ...
    def cos(target:float):
        ...
        inverse = False
        while target > 3.141592653589:
            target -= 6.283185307178
        if target > 1.5707963267945:
            inverse = True
            target -= 3.141592653589
        elif target < - 1.5707963267945:
            inverse = True
            target += 3.141592653589
        ...
        x = 39797 # parameter K6 << 16 : 0.6072529350088812561694 * 65536
        y = 0
        i = 6
        while i != -1:
            if direction[i] == 1:
        ...
        if inverse:
            return - x / 65536 # It never over than +-1, can be used in integer multiplication
        else:
            return x / 65536
...
def huffman_encode(block_list:list, double_word_list:list=[], last_double_word_space:int=0, mode=0):
    ...
    def push(space, push_bin, push_size):
        # assert push_size < 32 # debug usage, not appear in RISCV processor
    ...
    # print('DC: data', bin(data)[2:], 'data size:', data_size, 'code:', bin(code)[2:], 'code size:', code_size)
    push_bin = (code << data_size) + data # less than 32-bit: max code size=16, max data size=10
    push_size = data_size + code_size
    # print('Push:', bin(push_bin)[2:], 'size:', push_size, 'Space:', last_double_word_space)
    space = push(last_double_word_space, push_bin, push_size)
    # infor = '0'*(push_size - len(bin(push_bin)[2:])) + bin(push_bin)[2:]
    # if (len(double_word_list) > 2):
    #     print('Last Quater-Word Data', bin(double_word_list[-2])[2:], bin(double_word_list[-1])[2:])
    ...
    for index in range(1,64):
        if block_list[index] == 0:
            ...
            while zero_counter > 15: # zeros over than 15
                ...
                # print('Push:', bin(push_bin)[2:], 'size:', push_size, 'Space:', space)
                space = push(space, push_bin, push_size)
                # infor += '0'*(push_size - len(bin(push_bin)[2:])) + bin(push_bin)[2:]
                # print('Last Double-Word Data', bin(double_word_list[-1])[2:])
            # Assembly
            ...
            push_bin = (code << data_size) + data # less than 32-bit: max code size=16, max data size=10
            push_size = data_size + code_size
            # print('AC: zero/data', hex(ac_index)[2:], 'zeros:', zero_counter, 'data size:', data_size, 'code:', bin(code)[2:], 'code size:', code_size)
            # print('Push:', bin(push_bin)[2:], 'size:', push_size, 'Space:', space)
            space = push(space, push_bin, push_size)
            # infor += '0'*(push_size - len(bin(push_bin)[2:])) + bin(push_bin)[2:]
            # print('Last Quater-Word Data', bin(double_word_list[-2])[2:], bin(double_word_list[-1])[2:])
    ...
    if zero_counter != 0:
        ...
        # print('EOB Push:', bin(push_bin), push_size, 'Space:', space)
        space = push(space, push_bin, push_size)
        # infor += '0'*(push_size - len(bin(push_bin)[2:])) + bin(push_bin)[2:]
        # print('Last Double-Word Data', bin(double_word_list[-1])[2:])
    # print('Block Huffman code:', infor)
    # print('--------------------------\n')
