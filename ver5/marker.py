...
# JPEG Image file marker decode and encode
# Version 5.0
# Clark Pu
# -------------------------------------
...
def dqt_analysis(marker, show_infor=False):
...
def appn_analysis(marker, show_infor=False):
    table = read_in_byte(marker)
    if (table[0] + table[1]).upper() == APPn:
        ...
        if show_infor:
            ...
            print('\n-------------------------------------\n')
        return identifier, version, units, x_density, y_density, x_thumbnail, y_thumbnail, Thumbnail_data
    else:
        return False
    

# SOS: Start of Scan marker decode
def sos_analysis(marker, show_infor=False):
    table = read_in_byte(marker)
...
def sof_analysis(marker, show_infor=False):
    table = read_in_byte(marker)
    if (table[0] + table[1]).upper() == SOF0:
        ...
        for i in range(0, Nf):
            ...
            if show_infor:
                ...
                print('\n-------------------------------------\n')
        return Lf, P, Y, X, Nf, C, H, V, Tq
    else:
        return False


# DHT: define huffman table marker decode
def huffman_table_generate(marker, show_infor=False):
    ...
    if (table[0] + table[1]).upper() == DHT:
        ...
        if show_infor:
            ...
            display(BITS, 4, 16)
            print('\nHUFFVAL:')
            display(HUFFVAL, 4, 16)
            print('\nHUFFSIZE:')
            display(HUFFSIZE, 3, 16)
            print('\nHUFFCODE:')
            display(HUFFCODE, 6, 16)
            print('\nEHUFCO:')
            display(EHUFCO, 6, 16)
            print('\nEHUFSI:')
            display(EHUFSI, 3, 16)
            print('\n-------------------------------------\n')
        return EHUFCO, EHUFSI, Th
    else:
        return False


# From an image file read the marker
def readfile(file, marker_name, marker_index=0, show_infor=False):
